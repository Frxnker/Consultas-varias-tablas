...
-- 6 SQL2(INNER JOIN)
SELECT DISTINCT oficina.linea_direccion1, oficina.linea_direccion2, oficina.ciudad, oficina.region, oficina.pais, oficina.codigo_postal
FROM oficina
INNER JOIN empleado ON oficina.codigo_oficina = empleado.codigo_oficina
INNER JOIN cliente ON empleado.codigo_empleado_rep_ventas = cliente.codigo_empleado_rep_ventas
...
-- 8 SQL2(INNER JOIN)
SELECT peon.nombre AS empleado, patron.nombre As jefe
FROM empleado peon INNER JOIN empleado patron
ON peon.codigo_jefe = patron.codigo_empleado;

-- 9 SQL2(INNER JOIN)
SELECT e1.nombre AS empleado, e2.nombre AS jefe, e3.nombre AS boss
...
WHERE DATEDIFF(pedido.fecha_entrega, pedido.fecha_esperada) > 1;

-- 11 SQL2(INNER JOIN)
SELECT cliente.nombre_cliente, GROUP_CONCAT(DISTINCT producto.gama ORDER BY producto.gama) AS gamas_compradas
FROM cliente
